...
var internals = {},
  ...
  internals.index = async function (req, reply) {
    ...
    const company_id = req.auth.credentials._id;
    const staff_id = 0;
    const branchID = 0;
    const collectorID = 0;
    ...
    try{
      //GET STAFF OF THIS USER
      let myStaff = await Staffs.find({
      ...
      //REMOVE REPETATIVE
      const seen = new Set();
      const filteredArr = myStaff.filter(el => {
        const duplicate = seen.has(el.staff_id._id);
        seen.add(el.staff_id._id);
        return !duplicate;
      });
      ...
      reply.view("company/reports/index.html", {
      ...
      console.log(error)
      return reply.redirect("/company/reports?message=Error, try again!&alertType=error");
    }
  };
  ...
  internals.search = async function (req, reply) {
    ...
    if(isEmpty(branchID)|| isEmpty(collectorID)|| isEmpty(year) || isEmpty(req.query.startDate) || isEmpty(req.query.endDate)){
      return reply.redirect("/company/reports?message=Error, Please fillup the form correctly!&alertType=error");
    }
  
    try{
      //GET ALL STAFF OF THIS USER
      let myStaff = await Staffs.find({
        company_id: company_id
      })
      .populate('staff_id')
      ...
      if(parseInt(staff_id)!=0){
        var staffInfo = await Users.findOne({
          _id: staff_id
        }).lean();
